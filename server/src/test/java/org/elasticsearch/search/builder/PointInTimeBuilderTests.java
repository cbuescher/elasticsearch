...
public class PointInTimeBuilderTests extends AbstractXContentSerializingTestCase<PointInTimeBuilder> {
    ...
    public void testDecodePits() {

        String original = "";
        String updated = "";
        assertFalse(original.equals(updated));
        BytesReference pitId = new BytesArray(Base64.decodeBase64(original));
        BytesReference updatedPitId = new BytesArray(Base64.decodeBase64(updated));
        logger.info(
            "Original PIT: " + new PointInTimeBuilder(pitId).getSearchContextId(this.writableRegistry()).toString().replace("},", "\n")
        );
        // logger.info(
        // "Rewritten PIT: " + new PointInTimeBuilder(updatedPitId).getSearchContextId(this.writableRegistry()).toString().replace("},",
        // "\n")
        // );
    }
}
