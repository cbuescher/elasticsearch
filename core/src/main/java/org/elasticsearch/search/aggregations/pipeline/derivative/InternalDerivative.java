...
public class InternalDerivative extends InternalSimpleValue implements Derivative {
    ...
    public static InternalDerivative parseXContentBody(String name, XContentParser parser) {
        ...
        double value = (Double) map.getOrDefault(CommonFields.VALUE.getPreferredName(),
                Double.NEGATIVE_INFINITY);
        String valueAsString = (String) map.get(CommonFields.VALUE_AS_STRING.getPreferredName());
        double normalizedValue = (Double) map.getOrDefault(NORMALIZED_VALUE.getPreferredName(),
                Double.NEGATIVE_INFINITY);
        ...
        if (valueAsString != null) {
            Map<Object,String> valuesMap = new HashMap<>(2);
            valuesMap.put(value, valueAsString);
            valuesMap.put(normalizedValue, normalizedValueAsString);
            formatter = InternalNumericMetricsAggregation.wrapValueAsString(valuesMap);
        }
        return new InternalDerivative(name, value, value/normalizedValue, formatter, Collections.emptyList(), metaData);
    }
