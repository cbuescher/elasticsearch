...
public class RetrySearchIntegTests extends BaseSearchableSnapshotsIntegTestCase {
    ...
    public void testRetryPointInTime() throws Exception {
        ...
        SetOnce<BytesReference> pitInResponse = new SetOnce<>();
        SearchContextId pit1 = SearchContextId.decode(writableRegistry(), pitId);
        System.out.println(">>> " + Base64.getUrlEncoder().encodeToString(BytesReference.toBytes(pitId)));
        System.out.println(("PIT1 " + pit1).replace("\\},", "\\},\n"));
        ...
        try {
            ...
            for( String allocatedNode : allocatedNodes) {
                MockSearchService searchService = (MockSearchService) internalCluster().getInstance(SearchService.class,
                        allocatedNode);
                System.out.println("Active contexts on node " + allocatedNode + ": " + searchService.getActiveReaders());
            }
            for (String allocatedNode : allocatedNodes) {
                internalCluster().restartNode(allocatedNode);
                ensureGreen(indexName);
            }
            System.out.println("......... restarted nodes");
            for( String allocatedNode : allocatedNodes) {
                MockSearchService searchService = (MockSearchService) internalCluster().getInstance(SearchService.class,
                        allocatedNode);
                System.out.println("Active contexts on node " + allocatedNode + ": " + searchService.getActiveReaders());
            ...
            assertNoFailuresAndResponse(
                ...
                resp -> {
                    pitInResponse.set(resp.pointInTimeId());
                    assertHitCount(resp, docCount);
                }
            );

            SearchContextId pit2 = SearchContextId.decode(writableRegistry(), pitInResponse.get());
            System.out.println(">>> " + Base64.getUrlEncoder().encodeToString(BytesReference.toBytes(pitInResponse.get())));
            System.out.println(("PIT2 " + pit2).replace("\\},", "\\},\n"));

            System.out.println("......... finished first PIT search");
            ...
            for( String allocatedNode : allocatedNodes) {
                ...
                searchService.setOnPutContext(context -> {
            ...
            assertNoFailuresAndResponse(
                prepareSearch().setQuery(new RangeQueryBuilder("created_date").gte("2011-01-01").lte("2011-12-12"))
                    ...
                    .setPreFilterShardSize(between(1, 10))
                    .setAllowPartialSearchResults(randomBoolean())  // partial results should not matter here
                    .setPointInTime(new PointInTimeBuilder(pitInResponse.get())),
                resp -> {
                    SearchContextId pit3 = SearchContextId.decode(writableRegistry(), resp.pointInTimeId());
                    System.out.println(">>> " + Base64.getUrlEncoder().encodeToString(BytesReference.toBytes(resp.pointInTimeId())));
                    System.out.println(("PIT3 " + pit3).replace("\\},", "\\},\n"));


                    byte[] bytes1 = toBytes(resp.pointInTimeId());
                    byte[] bytes2 = toBytes(pitInResponse.get());
                    assertEquals(bytes1.length, bytes2.length);
                    for (int i = 0; i < bytes1.length; i++) {
                        assertEquals("difference at " + i, bytes1[i], bytes2[i]);
                    }
            ...
            client().execute(TransportClosePointInTimeAction.TYPE, new ClosePointInTimeRequest(pitInResponse.get())).actionGet();
        }
    }
}
