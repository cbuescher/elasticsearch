...
public class ParentToChildrenAggregatorTests extends AggregatorTestCase {
    ...
    /**
     * TODO: -Dtests.seed=2733C97E010AFC7E fails with
     * java.lang.ClassCastException: org.apache.lucene.search.QueryUtils$FCInvisibleMultiReader cannot be cast to org.apache.lucene.index.DirectoryReader
   >    at __randomizedtesting.SeedInfo.seed([2733C97E010AFC7E:2160722290CBCC66]:0)
   >    at org.elasticsearch.search.aggregations.support.ValuesSource$Bytes$ParentChild.globalMaxOrd(ValuesSource.java:166)
   >    at org.elasticsearch.search.aggregations.bucket.children.ChildrenAggregatorFactory.doCreateInternal(ChildrenAggregatorFactory.java:72)
   >    at org.elasticsearch.search.aggregations.bucket.children.ChildrenAggregatorFactory.doCreateInternal(ChildrenAggregatorFactory.java:39)
   >    at org.elasticsearch.search.aggregations.support.ValuesSourceAggregatorFactory.createInternal(ValuesSourceAggregatorFactory.java:55)
    ...
    private static List<Field> createParentDocument(String id) {
        return Arrays.asList(
                new StringField(TypeFieldMapper.NAME, PARENT_TYPE, Field.Store.NO),
    ...
    private static List<Field> createChildDocument(String childId, String parentId, int value) {
        return Arrays.asList(
                new StringField(TypeFieldMapper.NAME, CHILD_TYPE, Field.Store.NO),
    ...
    private void testCase(Query query, CheckedConsumer<RandomIndexWriter, IOException> buildIndex, Consumer<InternalChildren> verify)
        ...
        try (Aggregator aggregator = createAggregator(aggregationBuilder, indexSearcher, fieldType)) {
            ...
            InternalChildren result = (InternalChildren) aggregator.buildAggregation(0L);
            verify.accept(result);
            // TODO why do I manually have to close this? Its not closed by the outside try-catch?
            aggregator.subAggregator("in_child").close();
        }
        indexReader.close();
        directory.close();
    }
